...
// 1. 정보은닉
/*
	수정이 불가능하게 특정 정보를 숨기는 것을 정보은닉이라고 한다.
...
/*
	...
	복사할때 주소만 복사된 경우에는 안에 있는 값도 복사가 가능하도록 동적할당을 해주어야한다.
*/

// 9. 오버로딩 & 오버라이딩 - 어떻게 호출할 것인지가 핵심
/*
	오버로딩은 복사생성자와 같은 형태로 사용가능하고, 동일한 형태와 매개변수를 가졌을 경우 사용가능하다.
	매개변수가 다를 경우 다른 기능을 한다.
...
// 10. 연산자 오버로딩 - 연산자에 임의로 설정한 역할이 가능하도록 하는 것
struct tagInfo
{
	int Number;
